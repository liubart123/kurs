...
namespace Log
{
	...
	void WriteIn(LOG log, In::IN in)
	{
		...
		*log.stream << "Amount of lines: " << in.lines << endl;
		...
		bool kavichka2 = false;	//ц≥ Єсць адкрываюча€ кавычка адз≥нарна€
			...
			if (CheckSepSymb(in.text[pos + startPos]) && (kavichka == false && kavichka2==false)) {
				...
				if (pos != 0) {		//кал≥ ц€кучае слова не с≥мвал пропуска
					...
					if (result[0] == LEX_ERROR){
						result = ChrLiKA(tempString);//праверка на л≥тэрал с≥мвала
						if (result[0] == LEX_ERROR) {
							result = StrLiKA(tempString);//праверка на л≥тэрал радковы
							if (result[0] == LEX_ERROR) {
								result = IntLiKA(tempString);//праверка на л≥тэрал л≥чбавы dec
								if (result[0] == LEX_ERROR) {
									result = IntLiKAHex(tempString);//праверка на л≥тэрал л≥чбавы hex
									countSystem = LT::HEX;
									if (result[0] == LEX_ERROR) {
										result = IntLiKABin(tempString);//праверка на л≥тэрал л≥чбавы bin
										countSystem = LT::BIN;
										if (result[0] == LEX_ERROR) {
											result = IntLiKAOct(tempString);//праверка на л≥тэрал л≥чбавы oct
											countSystem = LT::OCT;
											if (result[0] == LEX_ERROR) {

...
namespace GRB {
	...
	short Rule::getNextChain(GRBHALPHABET* lenta, short pos, Rule::Chain& pchain, short j) {
		j++;
		for (; j < size; j++) {	//падбор прав≥ла пачынаецца з j
			int i=0;
			while (i<chains[j].size && chains[j].nt[i] == lenta[pos + i] 
			&& chains[j].isT(lenta[pos + i])==true
			&& chains[j].isT(chains[j].nt[i])==true) {
				i++;	//кал≥ с≥мвалы стужк≥ ≥ прав≥ла супадаюць ≥ з'€Ґлюцца л≥тэралам≥
			}
			if (i == chains[j].size) {
				break;	//кал≥ ҐсЄ прав≥ла адпав€дае стужцы
			}
			else if ((chains[j].nt[i] == lenta[pos + i] || chains[j].isT(chains[j].nt[i]) == false)) { 
				break;		//ланцужок прав≥ла ≥ стужка супадаюць
			}
		}
		short rc = j<size?j:-1;
		if (rc >= 0) {
	...
	Greibach greibach(NS('S'),TS('$'),7,	//стартавы нетырм≥нал, маркер дн€ стэка, колькасць прав≥л у граматыцы
